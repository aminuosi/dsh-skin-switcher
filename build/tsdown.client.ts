/**
 * Reproducible tsdown preset for this standalone dsh distribution, mirrored
 * from dsh-web-ui. Emits a closure-factory artifact: the bundle calls
 ...
 * loader removes plugin-owned tags on unload). The platform module list
 * mirrors the shell's seed table in `./web-platform.ts`.
 * @module @dsh-external/dsh-client-ui-skin-switcher/build/tsdown.client
 */
...
function clientConfig(id: string, entry: string, portableCssModuleIds: boolean): UserConfig {
  ...
  return {
    ...
    // loader module table must inline instead (wire/type layers, clsx — every
    ...
    // table entries (external above wins), bundle everything else.
